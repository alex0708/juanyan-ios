...
#import "Dai22bViewController.h"

#define KScreenHeight [UIScreen mainScreen].bounds.size.height
#define KScreenWidth [UIScreen mainScreen].bounds.size.width

@interface Dai22bViewController ()

@end

@implementation Dai22bViewController
    ...
    for (int i = 0; i<t; i++) {
        ...
        for (int j = 0; j<4; j++) {
            ...
            if (bLabel.tag == 102) {
                ...
                bLabel.text = @"3";
            }else if(bLabel.tag == 401) {
                bLabel.text = @"4";
            }else if(bLabel.tag == 501) {
                bLabel.text = @"5";
            }else if(bLabel.tag == 402) {
                bLabel.text = @"红河（全硬）";
            }else if(bLabel.tag == 403) {
                bLabel.text = @"50.000";
            }else if(bLabel.tag == 502) {
                bLabel.text = @"红河（全软）";
            }else if(bLabel.tag == 503) {
                bLabel.text = @"21.000";
            }
